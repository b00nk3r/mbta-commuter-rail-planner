    ...
    <div className="absolute top-1/3 left-6 transform -translate-y-1/2 z-[700] w-72 sm:w-80 rounded-2xl bg-white/90 backdrop-blur shadow-lg">
      <div className="p-6">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <h1 className="text-3xl font-bold text-[rgb(46,46,46)] relative top-[4px]" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>Trip Planner</h1>
        </div>

        <div className="mb-4">
          <label className="block text-sm font-regular text-[rgb(120,120,120)] uppercase mb-1" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>LINE</label>
          <div className="relative">
            <select value={selectedLine} onChange={onLineChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[1rem] appearance-none bg-white" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
          ...
          {!selectedLine && (<p className="text-red-600 mt-1 text-sm">Please select a line first</p>)}
        </div>

        <div className="mb-4">
          <label className="block text-sm font-regular text-[rgb(120,120,120)] uppercase mb-1" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>START STATION</label>
          <div className="w-full px-3 py-2 border border-gray-300 bg-gray-100 rounded-lg text-[1rem] bg-gray-100" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
            {startStation ? startStation : (selectedLine ? 'Determining...' : 'Select a line above')}
          </div>
        </div>

        <div className="mb-10">
          <label className="block text-sm font-regular text-[rgb(120,120,120)] uppercase mb-1" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>DESTINATION</label>
          <div className="relative">
            <select value={stopStation} onChange={onStopChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-[1rem] appearance-none bg-white" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }} disabled={!selectedLine || !startStation || sortedStationsForStop.length === 0}>
          ...
          {selectedLine && startStation && !isStopStationSelected && (<p className="text-red-600 mt-1 text-sm">Please select a stop station.</p>)}
        </div>

        <Link to="/tripSummaryPage">
          <button className="w-full px-3 py-3 rounded-lg text-[1.1rem] text-white font-bold bg-[rgb(160,68,118)] hover:bg-[rgb(140,58,108)] transition-colors duration-200 disabled:bg-[rgb(180,88,128)] disabled:cursor-not-allowed" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }} disabled={!isStopStationSelected}>
            Plan Trip
          </button>
        </Link>
      </div>
