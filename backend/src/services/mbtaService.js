...
// --- low-level API calls ---
async function getTrainRoutes() {
  const res = await axios.get(`${config.baseUrl}/routes`, {
...
async function getStopsForRoute(routeId) {
  const res = await axios.get(`${config.baseUrl}/stops`, {
...
async function getLineDetails(lineId) {
  const res = await axios.get(`${config.baseUrl}/routes/${lineId}`, {
    headers: getHeaders()
  });
  return res.data?.data;
}

async function getShapesForRoute(routeId) {
  const res = await axios.get(`${config.baseUrl}/shapes`, {
    params: { 'filter[route]': routeId },
    headers: getHeaders()
  });
  return { routeId, shapes: res.data?.data || [] };
}

async function getDepartures(origin, line) {
  const res = await axios.get(`${config.baseUrl}/schedules`, {
    params: {
      'filter[stop]': origin,
      'filter[route]': line,
      'filter[route_type]': 2,
  ...
  const data = res.data?.data || [];
  return data.map(s => ({
    departure_time: s.attributes.departure_time,
    train_number: s.relationships.trip.data.id,
    route_id: s.relationships.route.data.id
  }));
}

// --- transforms/helpers specific to your data ---
function dedupeStops(listOfStopsArrays) {
  const unique = new Map();
  listOfStopsArrays.flat().forEach(stop => {
    if (stop && !unique.has(stop.id)) {
      unique.set(stop.id, {
        _id: stop.id,
...
function attachImages(stops) {
  return stops.map(s => ({
    ...s,
    imageUrl: imageMap[s.stationName] || 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/64/MBTA.svg/768px-MBTA.svg.png'
  }));
}
...
function processShapes(routeShapesData) {
  const routeShapes = {};
  routeShapesData.forEach(({ routeId, shapes }) => {
    if (!routeShapes[routeId]) routeShapes[routeId] = [];
    (shapes || []).forEach(raw => {
      if (!raw?.id || !raw?.attributes?.polyline) return;
      routeShapes[routeId].push({
        _id: raw.id,
        routeId,
        polyline: raw.attributes.polyline
      });
    });
  });
  return routeShapes;
...
// --- high-level orchestrators used by controllers ---
async function listStops() {
  const routeIds = await getTrainRoutes();
  const promises = routeIds.map(getStopsForRoute);
  const allStops = await Promise.all(promises);
  const deduped = dedupeStops(allStops);
  return attachImages(deduped);
}

async function listLines() {
  const ids = await getTrainRoutes();
  const details = await Promise.all(ids.map(getLineDetails));
  return dedupeLines(details.filter(Boolean));
}

async function listShapesByRoute() {
  const ids = await getTrainRoutes();
  const shapes = await Promise.all(ids.map(getShapesForRoute));
  return processShapes(shapes);
}
